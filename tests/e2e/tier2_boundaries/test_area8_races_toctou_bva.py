...
class TestArea8RacesTOCTOUBVA(unittest.TestCase):
    ...
    def test_e2e_t2_toctou_journal_append_race(self) -> None:
        """Verify concurrent threads appending journal entries produce valid append-only journal."""
        with TempSessionWorkspace() as ws:
            session_id = "journal-race-001"
            rescue_root = ws.root / "rescues"

            def _append_worker(worker_id: int) -> None:
                for i in range(10):
                    entry = JournalEntry(
                        version=1,
                        session_id=session_id,
            ...
            threads = [threading.Thread(target=_append_worker, args=(w,)) for w in range(4)]
            for t in threads:
                t.start()
            for t in threads:
                t.join()
